...
bb_daily_target_test()
{
  ...
  bb_make binutils "-j`nproc` check RUNTESTFLAGS=--target_board=arm-sim"

  export PATH=${PREFIX}/bin:${PATH}

  # GCC pass 1: no libc yet
  bb_config gcc "--target=arm-none-eabi --with-newlib --without-headers --enable-languages=c --enable-checking=yes,rtl --disable-libssp"
  bb_make gcc "-j`nproc`"
  bb_make gcc "install"

  # Libc
  bb_config newlib "--target=arm-none-eabi --enable-newlib-io-long-long --enable-newlib-io-long-double --enable-newlib-io-c99-formats"
  bb_make newlib "-j`nproc`"
  bb_make newlib "install"

  # GCC pass 2: full feature set
  bb_config gcc "--target=arm-none-eabi --with-newlib --enable-languages=c,c++ --enable-checking=yes,rtl"
  bb_make gcc "-j`nproc`"
  bb_make gcc "install"

  # Test newlib
  bb_make newlib "-j`nproc` check RUNTESTFLAGS=--target_board=arm-sim"

  # Test GCC
  bb_make gcc "-j`nproc` check-gcc-c RUNTESTFLAGS=--target_board=arm-sim"
  bb_make gcc "-j`nproc` check-gcc-c++ RUNTESTFLAGS=--target_board=arm-sim"
  ...
  bb_make --ignore-errors binutils "-j`nproc` check RUNTESTFLAGS=--target_board=arm-sim"

  # Save all the logs
  bb_gather_log_files ${BUILD_TAG}

  # Send to real mailing list,
  pushd ${WORKSPACE}/arm-gcc-build || error "failed to enter arm-gcc-build"
