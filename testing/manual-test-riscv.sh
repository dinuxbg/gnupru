...
bb_daily_target_test()
{
  ...
  bb_make binutils "-j`nproc` check RUNTESTFLAGS=--target_board=riscv-sim"
  ...
  # Test newlib
  bb_make newlib "-j`nproc` check RUNTESTFLAGS=--target_board=riscv-sim"

  # Test GCC
  bb_make gcc "-j`nproc` check-gcc-c RUNTESTFLAGS=--target_board=riscv-sim"
  bb_make gcc "-j`nproc` check-gcc-c++ RUNTESTFLAGS=--target_board=riscv-sim"
  ...
  bb_make --ignore-errors binutils "-j`nproc` check RUNTESTFLAGS=--target_board=riscv-sim"
